pipeline {
    ...
    stages {
        stage('build') {
            steps {
                sh 'docker run --name 6232ba0d5ba5_$BUILD_TAG -i --rm -v jenkins_jenkins:/repo 6232ba0d5ba5 phpunit --bootstrap /repo/workspace/$JOB_NAME/src/Email.php /repo/workspace/$JOB_NAME/tests/EmailTest.php'
